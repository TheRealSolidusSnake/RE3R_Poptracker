-- archipelago.lua (fixed flattened ITEM_MAPPING handling)
...
function onClear(slot_data)
    ...
    -- reset items
    -- ITEM_MAPPING is expected to be flattened: [id] = {"code", "type"} 
    -- But we still support nested lists { {"code","type"}, {"code2","type"} }
    for _, item_entry in pairs(ITEM_MAPPING) do
        if type(item_entry[1]) == "string" then
            -- single mapping
            local item_code = item_entry[1]
            local item_obj = Tracker:FindObjectForCode(item_code)
            if item_obj then
                if item_obj.Type == "toggle" then
                    item_obj.Active = false
                elseif item_obj.Type == "progressive" then
                    -- preserve original behavior: set to 0 then +1 (if that's intended)
                ...
                -- helpful debug if code missing
                print(string.format("onClear: could not find object for code '%s' (single mapping)", tostring(item_code)))
            end
        else
            -- list of mappings
            for _, mapping in ipairs(item_entry) do
                if type(mapping) == "table" and type(mapping[1]) == "string" then
                    local item_code = mapping[1]
                    local item_obj = Tracker:FindObjectForCode(item_code)
                    if item_obj then
                        if item_obj.Type == "toggle" then
                            item_obj.Active = false
                        ...
                        print(string.format("onClear: could not find object for code '%s' (list mapping)", tostring(item_code)))
                    end
                end
            end
        end
...
function onItem(index, item_id, item_name, player_number)
    ...
    -- Support both flattened single mapping and lists of mappings
    if type(item[1]) == "string" then
        -- single mapping {"code","type",maybe_qty}
        ...
        if item_obj then
            ...
            print(string.format("onItem: could not find object for code %s (single mapping)", tostring(item_code)))
        end
    else
        -- list mapping { {"code","type",qty}, ... }
        for _, item_pair in ipairs(item) do
            if type(item_pair) == "table" and type(item_pair[1]) == "string" then
                ...
                if item_obj then
                    ...
                    print(string.format("onItem: could not find object for code %s (list mapping)", tostring(item_code)))
                end
            end
        end
    end
...
-- updateHints(locationID, status, clear) where `clear` is optional (defaults false)
function updateHints(locationID, status, clear)
    ...
    if location_table then
        for _, location in ipairs(location_table) do
            ...
            if obj then
                obj.Highlight = highlight_lvl[status] or Highlight.Unspecified
            else
                print(string.format("No object found for code: %s", location))
            end
        end
    else
        print(string.format("updateHints: no location mapping for locationID %s", tostring(locationID)))
    end

    local item_codes = HINTS_MAPPING and HINTS_MAPPING[locationID] or nil
    if item_codes then
